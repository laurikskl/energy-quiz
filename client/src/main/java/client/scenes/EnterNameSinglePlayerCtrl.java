...
public class EnterNameSinglePlayerCtrl {
  ...
  public void startGame(ActionEvent actionEvent) throws IOException {
    ...
    if (usernameString.isEmpty()) {
      ...
      URL url =
          new File("client/src/main/resources/client/scenes/SPGameScreen.fxml").toURI().toURL();
      FXMLLoader loader = new FXMLLoader(url);
      Parent root = loader.load();

      SPGameController spGameController = loader.getController();
      spGameController.initialize(
          usernameString); //here we initialize the SP screen with the name the user has provided in the top center
